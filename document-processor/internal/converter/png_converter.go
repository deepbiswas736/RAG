...
// PNGConverter implements the Converter interface for PNG image files
type PNGConverter struct{}

// NewPNGConverter creates a new PNG converter
func NewPNGConverter() *PNGConverter {
	return &PNGConverter{}
}

// Convert converts a PNG image to PDF
func (c *PNGConverter) Convert(document *model.Document, output io.Writer) error {
	fmt.Println("Converting PNG image to PDF:", document.Name)

	// Read the content from the document
	content, err := io.ReadAll(document.Content)
	if err != nil {
		return fmt.Errorf("failed to read PNG content: %w", err)
	}
	...
	tempFile := filepath.Join(os.TempDir(), document.ID+".png")
	if err := os.WriteFile(tempFile, content, 0644); err != nil {
		return fmt.Errorf("failed to write temporary PNG file: %w", err)
	}
...
func (c *PNGConverter) CanConvert(document *model.Document) bool {
	return document.Format == model.FormatPNG
}
